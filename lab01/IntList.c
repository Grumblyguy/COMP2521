 int// IntList.c - Lists of integers
   ...
   if(L->size == 0){
      ...
      if(curr->next == NULL){
         ...
         while(curr->next != NULL){
            if(v <= curr->data){
               struct IntListNode *temp = newIntListNode(v);   //Creating the new node
               struct IntListNode *oldnext = curr->next;       // Saving address of original next
               curr->next = temp;            
               temp->next = oldnext;
         ...
         if(v < curr->data){
            IntListInsert(L, curr->data); //Should switch the last two around
            curr->data = v; 
            changes++;              
         }        
         next:
         ...
         if(changes == 0){
            IntListInsert(L, v);    //Here if most recent number is the greatest
         }   
      }     
  }
