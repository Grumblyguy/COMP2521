...
void BSTreeLevelOrder (BSTree t)
{
    ...
    while (QueueIsEmpty(New) == 0){
        if(New->head->value != NULL){
            printf("%d ", New->head->value->value);
        }
        if(New->head->value != NULL){
            if (New->head->value->left != NULL){
                QueueJoin(New, New->head->value->left);
            }
            if(New->head->value->right != NULL){
                QueueJoin(New, New->head->value->right);
            }
        }
        t  =  QueueLeave(New);
    }
    dropQueue(New);
    //dropqueue free memory
    //showqueue displays it
    //queuejoin adds
    //queueleave() removews
...
int BSTreeNumLeaves (BSTree t)
{
    if (t == NULL){
        ...
        return BSTreeNumLeaves(t->right) + BSTreeNumLeaves(t->left);
    }    
