...
int connections (Map g, LocationID start, LocationID end, TransportID type[])
{
    ...
    assert (g != NULL);
    int counter = 0;
    int connections = 0; //Number of connections between the two places
  
    while (counter < NUM_MAP_LOCATIONS){        
        printf("counter = %d\n", counter);
        char *name1 = idToName(start);
        char *name2 = idToName(end);
        char *name3 = idToName(g->connections[counter]->v);
        char *name4 = idToName(g->connections[counter]->next->v);
        
        printf("start = %s, end = %s, counter = %s, next = %s\n", name1,name2,name3,name4);

        if( ((strcmp(name1,name3) == 0) && strcmp(name2,name4) == 0) || ((strcmp(name2,name3) == 0) && (strcmp(name1,name4) == 0))){    
            printf("inside if statement\n");
            connections++;
            type[connections-1] = g->connections[counter]->type;
